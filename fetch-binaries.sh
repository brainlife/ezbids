...
usage() {
    ...
    echo "  platform: darwin | linux | windows (target platform; can fetch any platform from any host)" >&2
...
if [[ -z "$platform" ]] || [[ ! "$platform" =~ ^(darwin|linux|windows)$ ]]; then
    echo "Error: first argument must be darwin, linux, or windows" >&2
    usage
fi
...
matching=$(
    echo "$release_json" | jq -r --arg p "$platform" --arg a "$arch" '
...
if [[ "$platform" == "windows" && "$arch" == "amd64" ]]; then
    ...
    if [[ "$matching" != *"$sevenz_exe"$'\t'* ]]; then
        if [[ "$sevenz_exe_url" != *"/null" ]]; then
            matching="${matching}"$'\n'"${sevenz_exe}"$'\t'"${sevenz_exe_url}"
        else
            echo "Missing required Windows 7z asset: ${sevenz_exe}" >&2
            exit 1
        fi
    fi
