...
import type { WorkflowDefinition, WorkflowRunState, Binding } from './workflowTypes';
import { getWorkflowDefinitions } from './workflowLoader';
import { getToolExecutor } from './toolRegistry';
import { getHeuristic } from './heuristicRegistry';
...
export async function executeStep(runId: string, stepName: string): Promise<Record<string, unknown>> {
    const state = activeRuns.get(runId);
    if (!state) throw new Error(`No active run: ${runId}`);
