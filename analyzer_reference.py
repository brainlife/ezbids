...
for s in range(len(subjects)):
    ...
    for i in range(len(sub_protocol)):
        ...
        for j in range(len(section_indices)):
            ...
            if any(x in ['magnitude1', 'magnitude2', 'phasediff'] for x in modality_labels[section_start:section_end]):
        ...
        if len(fmap_intended_for_list) > 0:
            ...
            fmap_counter += 1
            if fmap_counter > 1 and 'magnitude' not in sub_protocol[i]['br_type']:
...
def fmap_intended_for(sub_protocol):
    ...
    Parameters
    ----------
    sub_protocol: list of dictionary, containing pertinent information needed 
        for the UI side of ezBIDS
    
    Returns
    ----------
    sub_protocol: same as above, but with updated information from the checks
    '''
    ...
    for j,k in enumerate(section_indices):
        ...
        for x,y in enumerate(br_types[section_start:section_end]):
            ...
            #Magnitude/Phasediff fmaps
                ...
                fmap_magphase_indices = [k+x for x, y in enumerate(br_types[section_start:section_end]) if y in ['fmap/magnitude1', 'fmao/magnitude2', 'phasediff']]
                bold_indices = [k+x for x, y in enumerate(br_types[section_start:section_end]) if y == 'fmap/bold' and include[k+x] != False]
