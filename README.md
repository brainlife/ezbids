...
DICOM files or dcm2niix output, and analyze the directory structure and sidecars generated from dcm2niix in order to *infer* 
...
Users do not need to organize their DICOM directory in any specific manner, and may compress (e.g. zip, tar) their upload directory if they so choose. In lieu of DICOMs, users may instead upload a directory containing the NIFTI/JSON (and bval/bvec) files generated by dcm2niix, if they do not have access to the original DICOMs. Only DICOM files should be included in the uploaded directory, or only dcm2niix-generated NIFTI/JSON (and bval/bvec) files should be included in the uploaded directory, **not both**.
