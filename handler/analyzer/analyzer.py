...
def correct_pe(pe_direction, ornt):
    ...
    # pe_direction xyz (correction required)
        ...
        if pe_direction[0] == 'x':
            ...
            ValueError('''pe_direction does not contain letter i, j, k, x, y,
                       or z''')
...
def create_screenshots(nifti_path, num_volumes):
    '''
    ...
    plt.savefig('{}.png'.format(nifti_path[:-7]), bbox_inches='tight')
...
def generate_dataset_list(uploaded_files_list):
    ...
    for index, json_file in enumerate(json_list):
        ...
        try:
            volume_count = nib.load(json_file[:-4] + 'nii.gz').shape[3]
        except:
            volume_count = 1
...
def determine_subj_ses_IDs(dataset_list):
    ...
    subject_ids_info = list({'subject':x['subject'],
                             ...
                             'phenotype':{
                                 ...
                                 'age':x['PatientAge']},
                             'exclude': False,
                             'sessions': []} for x in dataset_list)
...
def identify_series_info(dataset_list_unique_series):
    ...
    for index, unique_dic in enumerate(dataset_list_unique_series):
        ...
        if any(x in sd for x in localizer_keys) or sd == 'tfl':
            ...
            unique_dic['message'] = '''Acquisition is believed to be a localizer
                because "{}" is in the SeriesDescription. Please modify if
                incorrect.'''.format([x for x in localizer_keys if re.findall(x, sd)][0])
        ...
        # Arterial Spin Labeling (ASL)
            ...
            unique_dic['error'] = 'Acqusition appears to be ASL, which is \
                currently not supported by ezBIDS at this time, but will be \
                in the future'
            unique_dic['message'] = 'Acquisition is believed to be asl/asl \
                because "{}" is in the SeriesDescription. Please modify if \
                incorrect. Currently, ezBIDS does not support ASL conversion \
                to BIDS'.format([x for x in asl_keys if re.findall(x, sd)][0])
        ...
        # Angiography
            ...
            unique_dic['error'] = 'Acqusition appears to be an Angiography \
                acquisition, which is currently not supported by ezBIDS at \
                this time, but will be in the future'
            unique_dic['message'] = 'Acquisition is believed to be anat/angio \
                because "{}" is in the SeriesDescription. Please modify if \
                incorrect. Currently, ezBIDS does not support Angiography \
                conversion to BIDS'.format([x for x in angio_keys if re.findall(x, sd)][0])
        ...
        # Magnitude/Phase[diff] and Spin Echo (SE) field maps
            ...
            if 'EchoNumber' in unique_dic['sidecar']:
                if any(x in unique_dic['json_path'] for x in ['_real.', '_imaginary.']):
                    unique_dic['error'] = 'Acquisition appears to be a real or \
                    ...
                    unique_dic['message'] = 'Acquisition is believed to be \
                        ...
                        incorrect'.format([x for x in se_mag_phase_fmap_keys if re.findall(x, sd)][0])
                    ...
                    unique_dic['message'] = 'Acquisition is believed to be \
                        ...
                        Please modify if incorrect'.format([x for x in se_mag_phase_fmap_keys if re.findall(x, sd)][0])
                    ...
                    unique_dic['error'] = 'Acquisition appears to be some form \
            ...
            # Spin echo field maps
                ...
                unique_dic['message'] = 'Acquisition is believed to be fmap/epi \
                    because "{}" is in SeriesDescription, and does not \
                    contain metadata info associated with magnitude/phasediff \
                    acquisitions. Please modify if incorrect'.format([x for x in se_mag_phase_fmap_keys if re.findall(x, sd)][0])
        ...
        # DWI
            ...
            series_entities['direction'] = unique_dic['direction']
            unique_dic['message'] = 'Acquisition appears to be a fmap/epi meant \
                for dwi/dwi, as "DIFFUSION" is in ImageType, and "b0" is in \
                the SeriesDescription. Please modify if incorrect'

        elif not any('.bvec' in x for x in unique_dic['paths']) and 'DIFFUSION' in unique_dic['ImageType']:
            unique_dic['error'] = 'Acquisitions has "DIFFUSION" label in the \
            ...
            if 'DIFFUSION' not in unique_dic['ImageType']:
                if unique_dic['NumVolumes'] < 2:
                    if any(x in sd for x in flair_keys):
                        ...
                        unique_dic['message'] = 'Acquisition is believed to be \
                            ...
                            SeriesDescription. Please modify if incorrect'.format([x for x in flair_keys if re.findall(x, sd)][0])
                    elif 't2w' in sd:
                        unique_dic['DataType'] = 'anat'
                        unique_dic['ModalityLabel'] = 'T2w'
                        unique_dic['message'] = 'Acquisition is believed to be \
                            anat/T2w because "t2w" is in the \
                            SeriesDescription. Please modify if incorrect'
                    else:
                        unique_dic['error'] = 'Acquisition has bval and bvec \
                            ...
                            otherwise will not convert to BIDS'
                        unique_dic['message'] = unique_dic['error']
                        unique_dic['br_type'] = 'exclude'
                else:
                    unique_dic['DataType'] = 'dwi'
                    unique_dic['ModalityLabel'] = 'dwi'
                    unique_dic['message'] = 'Acquisition appears to be dwi/dwi \
                        because although "DIFUSSION" is not in ImageType, the \
                        acquisition has bval and bvec files and has {} \
                        volumes. Please modify if incorrect'.format(unique_dic['NumVolumes'])
                    series_entities['direction'] = unique_dic['direction']
            else:
                ...
                if np.max(bval) <= 50:
                    ...
                    series_entities['direction'] = unique_dic['direction']
                    unique_dic['forType'] = 'dwi/dwi'
                    unique_dic['message'] = 'Acquisition appears to be \
                ...
                # elif any(x in sd for x in dwi_derived_keys) and not any(x in sd for x in dwi_keys):
                    ...
                    unique_dic['error'] = 'Acquisition appears to be a TRACE, \
                    ...
                    unique_dic['message'] = 'Acquisition is believed to be \
                        ...
                        incorrect'.format([x for x in dwi_derived_keys if re.findall(x, sd)][0])
                    unique_dic['br_type'] = 'exclude'
                else:
                    unique_dic['DataType'] = 'dwi'
                    unique_dic['ModalityLabel'] = 'dwi'
        ...
        # DWI derivatives or other non-BIDS diffusion offshoots
            ...
            unique_dic['error'] = 'Acquisition appears to be a TRACE, FA, or \
                ADC, which are unsupported by ezBIDS and will therefore not \
                be converted'
            unique_dic['message'] = 'Acquisition is believed to be dwi-derived \
        ...
        # Functional bold and phase
            ...
            if unique_dic['NumVolumes'] < 50:
                unique_dic['br_type'] = 'exclude'
                unique_dic['message'] = 'Acquisition appears to be functional \
                ...
                if any(x in sd for x in func_rest_keys):
                    ...
                    unique_dic['message'] = 'Acquisition is believed to be \
                        ...
                        (but not "sbref"). Please modify if incorrect'.format([x for x in func_keys if re.findall(x, sd)][0])
                if 'MOSAIC' and 'PHASE' in unique_dic['ImageType']:
                    ...
                    unique_dic['message'] = 'Acquisition is believed to be \
                        ...
                        Please modify if incorrect'.format([x for x in func_keys if re.findall(x, sd)][0])
                else:
                    unique_dic['ModalityLabel'] = 'bold'
                    if unique_dic['EchoNumber']:
                        ...
                        unique_dic['message'] = 'Acquisition is believed to be \
                            ...
                            EchoTime == {}. Please modify if incorrect'.format([x for x in func_keys if re.findall(x, sd)][0], unique_dic['EchoNumber'])
                if unique_dic['EchoNumber']:
                    series_entities['echo'] = unique_dic['EchoNumber']
                    unique_dic['message'] = 'Acquisition is believed to be \
                        multiecho func/bold because "{}" is in the \
        ...
        # Functional single band reference (sbref)
            ...
            if 'DIFFUSION' in unique_dic['ImageType']:
                ...
                if unique_dic['EchoNumber']:
                    series_entities['echo'] = unique_dic['EchoNumber']
                unique_dic['message'] = 'Acquisition is believed to be \
                    func/sbref because "sbref" is in the SeriesDescription'
        ...
        # T1w
            ...
            if 'multiecho' in sd or 'echo' in sd:
                if 'MEAN' not in image_type:
                    series_entities['echo'] = unique_dic['EchoNumber']
            unique_dic['message'] = 'Acquisition is believed to be anat/T1w \
                because "{}" is in the SeriesDescription. Please modify if \
                incorrect'.format([x for x in t1w_keys if re.findall(x, sd)][0])

        # FLAIR
        elif any(x in sd for x in flair_keys):
            unique_dic['DataType'] = 'anat'
            unique_dic['ModalityLabel'] = 'FLAIR'
            unique_dic['message'] = 'Acquisition is believed to be anat/FLAIR \
                because "{}" is in the SeriesDescription. Please modify if \
                incorrect'.format([x for x in flair_keys if re.findall(x, sd)][0])
        ...
        #T2w acquisitions typically have EchoTime > 100ms
            ...
            unique_dic['DataType'] = 'anat'
            unique_dic['ModalityLabel'] = 'T2w'
            unique_dic['message'] = 'Acquisition is believed to be anat/T2w \
                because "{}" is in the SeriesDescription and EchoTime > 100ms. \
                Please modify if incorrect'.format([x for x in t2w_keys if re.findall(x, sd)][0])
        ...
        if 'anat' in unique_dic['br_type'] and not any(x in ['DERIVED', 'NORM'] for x in unique_dic['ImageType']):
            unique_dic['br_type'] = 'exclude'
            unique_dic['error'] = 'Acquisition is a poor resolution {} \
                (non-normalized); Please check to see if this {} acquisition \
                should be converted to BIDS. Otherwise, this object will not \
                be included in the BIDS output'.format(unique_dic['br_type'], unique_dic['br_type'])
            unique_dic['message'] = unique_dic['error']
        ...
        print('Unique data acquisition file {}, Series Description {}, was \
              determined to be {}'.format(unique_dic['nifti_path'], unique_dic['SeriesDescription'], unique_dic['br_type']))
        print('')
        print('')
    ...
    for s_list in unique_series_list:
        if 'sbref' in s_list['type']:
            ...
            if corresponding_file['type'] == 'exclude':
                s_list['message'] = 'The corresponding file to this sbref is \
            ...
            if 'anat' or 'fmap' in corresponding_file['type']:
                s_list['message'] = 'The corresponding file to this sbref is \
                    ...
                    sbref will be excluded. Please modify if incorrect'
    ...
    for unique_subj_ses in unique_subj_ses_pairs:
        ...
        for p, protocol in enumerate(scan_protocol):
            ...
            if object_img_array.dtype not in ['<i2', '<u2']:
                ...
                create_screenshots(protocol['nifti_path'], protocol['NumVolumes'])
