...
import bidsEntitiesOrdered = require('../ui/src/assets/schema/rules/entities.json');

import { IObject } from '../ui/src/store';
...
const keys = ['participant_id'];
for (const key in info.participantsColumn) {
    keys.push(key);
}
const tsv = [];
const tsvheader = [];
...
const tsvf = fs.openSync(root + '/bids/' + datasetName + '/participants.tsv', 'w');
for (const rec of tsv) {
    fs.writeSync(tsvf, rec.join('\t') + '\n');
}
fs.closeSync(tsvf);

//handle each objects
async.forEachOf(info.objects, (o: IObject, idx, next_o) => {
    ...
    const typeTokens = o._type.split('/');
    const modality = typeTokens[0]; //func, dwi, anat, etc.. (or exclude)
    const suffix = typeTokens[1]; //t1w, bold, or "objN" for exclude)

    //construct basename
    const tokens = [];
    //for(let k in o._entities) {
    for (const k in info.entityMappings) {
    ...
    function handleItem(item, filename, derivatives = null) {
        ...
        //setup directory
        const fullpath = root + '/' + path + '/' + name + '_' + filename;
            ...
            switch (item.name) {
                ...
                case 'json':
                    ...
                    if (o.IntendedFor) {
                        ...
                        for (const idx of o.IntendedFor) {
