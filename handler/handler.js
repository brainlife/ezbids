...
console.log("---------------------------------------");
ezbids_shared_1.models.connect(err => {
    if (err)
        throw err;
    run();
});
function run() {
    ezbids_shared_1.models.Session.find({
        ...
        yield handle(session, "./preprocess.sh", "preprocess", cb => {
            ...
            fs.readFile(workdir + "/ezBIDS_core.json", "utf8", (err, data) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    const ezbids = new ezbids_shared_1.models.ezBIDS({
    ...
    return new Promise((resolve, reject) => {
        session.save().then(() => {
            try {
                let monitor;
                let workdir = process.env.WORK_DIR + "/" + session._id;
                const p = (0, child_process_1.spawn)(script, [workdir], { cwd: __dirname, detached: true });
